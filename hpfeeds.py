...
# Workaround for python3 compatibility
def buffer_(obj, offset, size=None):
    m = memoryview(obj)
    if size:
        return m[offset:offset+size].tobytes()
    else:
        return m[offset:].tobytes()
...
#		data = data.encode('latin1')
	return msghdr(OP_PUBLISH, struct.pack('!B', len(ident)) + bytes(ident, 'ascii') + struct.pack('!B', len(chan)) + bytes(chan, 'ascii') + bytes(data, 'ascii'))
def msgsubscribe(ident, chan):
	return msghdr(OP_SUBSCRIBE, struct.pack('!B', len(ident)) + ident + chan)
def msgauth(rand, ident, secret):
	#rand   = rand.encode('utf8')
	secret = bytes(secret, 'ascii')
	hash_  = hashlib.sha1(rand+secret).digest()
	return msghdr(OP_AUTH, struct.pack('!B', len(ident)) + bytes(ident, 'ascii') + hash_)
...
class HPC(object):
	...
	def connect(self):
		...
		logger.debug('connecting to {0}:{1}'.format(self.host, self.port))
		...
		for opcode, data in self.unpacker:
			if opcode == OP_INFO:
				rest = buffer(data, 0)
				name, rest = rest[1:1+rest[0]], buffer(rest, 1+rest[0])
				rand = rest
				...
				try:
					...
					for opcode, data in self.unpacker:
						if opcode == OP_PUBLISH:
							rest = buffer(data, 0)
							ident, rest = rest[1:1+ord(str(rest[0]))], buffer(rest, 1+ord(str(rest[0])))
							chan, content = rest[1:1+ord(str(rest[0]))], buffer(rest, 1+ord(str(rest[0])))
